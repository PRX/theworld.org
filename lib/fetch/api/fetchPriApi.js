...
const fetchPriApiQuery = async (type, params = false) =>
  fetchPriApi(type, params).then(resp => !resp.isFailure && resp.response);

const fetchPriApiItem = async (type, id, params = false) =>
  fetchPriApi(`${type}/${id}`, params).then(resp => !resp.isFailure && resp.response);
